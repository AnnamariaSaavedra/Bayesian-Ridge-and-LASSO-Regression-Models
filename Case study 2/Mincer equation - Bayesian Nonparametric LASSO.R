...
# 3. Bayesian Nonparametric LASSO regression
...
K <- apply(M5$XI, 1, function(x) length(unique(x))) # Compute the number of clusters on each iteration
...
hat_np <- function(model, K, p){
  ...
  return(list(inference = inf, beta = beta, sigma2 = sigma2))
}

inference <- hat_np(M5, K = 2, p)

# Bayesian inference for lambda

LAMBDA_MEAN <- round(mean(M5$LAMBDA), 3) # Posterior mean

LAMBDA_MEDIAN <- round(median(M5$LAMBDA), 3) # Posterior median

LAMBDA_SD <- round(sd(M5$LAMBDA), 3) # Posterior standard deviation

CI_LAMBDA <- round(quantile(x = M5$LAMBDA, probs = c(0.025, 0.975)), 3) # 95% credible interval

# Bayesian inference for alpha

ALPHA_MEAN <- round(mean(M5$ALPHA), 3) # Posterior mean

ALPHA_MEDIAN <- round(median(M5$ALPHA), 3) # Posterior median

ALPHA_SD <- round(sd(M5$ALPHA), 3) # Posterior standard deviation

CI_ALPHA <- round(quantile(x = M5$ALPHA, probs = c(0.025, 0.975)), 3) # 95% credible interval
...
xi_hat_cv <- matrix_A(model = M5_CV, k_pos, n = length(y_train)) # Since K = 1, all the observations belong to the same cluster
...
# Plot the histogram
hist(x = y, freq = FALSE,
     ylab = "Densidad", main = "", xlim = c(11, 18), ylim = c(0, 1.2),
